...
def build_suite(world, pool, vehicle, save_dir=None):
    ...
    imu = pool.add(world.spawn_actor(
        bp_lib.find("sensor.other.imu"), where["imu"], attach_to=vehicle,
        attachment_type=carla.AttachmentType.Rigid))
    imu.listen(lambda d: counts.update(imu=1))
    sensors["imu"] = imu

    gnss = pool.add(world.spawn_actor(
        bp_lib.find("sensor.other.gnss"), where["gnss"], attach_to=vehicle,
        attachment_type=carla.AttachmentType.Rigid))
...
def main() -> None:
    ...
    ap.add_argument("--save-dir", default=None,
                    help="write every 20th camera frame here")
    ...
    with synchronous(world), ActorPool(client) as pool:
        ...
        print(f"ego: {vehicle.type_id}, half-extent "
              f"x={e.x:.2f} y={e.y:.2f} z={e.z:.2f} m")
        ...
        if counts["lidar"]:
            print(f"  mean LiDAR points per sweep: "
                  f"{counts['lidar_points'] / counts['lidar']:.0f}")
        if counts["radar"]:
            print(f"  mean RADAR detections per scan: "
